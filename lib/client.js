...
setTimeout(() => {
  ...
  async function F() {
    ...
    const {
      state: z,
      saveCreds: g
    } = await useMultiFileAuthState(__dirname + '/ROYAL_Baileys/');
    ...
    n.ev.on("messages.upsert", async B => {
      ...
      try {
        ...
        if (w && Config.HANDLERS.toLowerCase().includes('null')) {
          ...
          d = h && prefixRegex.test(h[0x0]) || a.split('@')[0x0] !== "923402054847" && m.isROYAL && h[0x0] == ',';
        ...
        if (B == "order") {
          m = {
            'orderMessage': {
              ...
              'message': "❏ " + h,
              'orderTitle': 'live',
              'sellerJid': '923402054847@s.whatsapp.net'
            }
          };
        } else {
          if (B == 'contact') {
...
async function MakeSession() {
  ...
  if (E) {
    SmdOfficial = "yes";
    if (!fs.existsSync(__dirname + '/ROYAL_Baileys/')) {
      fs.mkdirSync(__dirname + '/ROYAL_Baileys/');
    }
    ...
    if (cc && /guru/gi.test(cc) && cc.length < 0x1e) {
      try {
        ...
        fs.writeFileSync(__dirname + '/ROYAL_Baileys/' + "creds.json", JSON.stringify(a, null, 0x2));
      ...
      if (cc && cc.length > 0x3 && cc.length < 0x14) {
        try {
          ...
          fs.writeFileSync(__dirname + '/ROYAL_Baileys/' + "creds.json", atob(O), 'utf8');
        } catch (W) {
          console.log("\nERROR GETTING SESSION_ID FROM PASTE SERVER\n");
        }
      } else {
        if (cc) {
          try {
            ...
            if (o['creds.json']) {
              for (const x in o) {
                try {
                  fs.writeFileSync(__dirname + '/ROYAL_Baileys/' + x, typeof o[x] == 'string' ? o[x] : JSON.stringify(o[x], null, 0x2));
                } catch (S) {}
              }
            } else {
              fs.writeFileSync(__dirname + '/ROYAL_Baileys/' + "creds.json", JSON.stringify(o, null, 0x2));
            }
            console.log("\nCredentials Saved Successfully.");
          } catch (T) {
