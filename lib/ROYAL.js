...
(function () {
  d(this, function () {
    ...
    const H = G("init");
    if (!r.test(H + "chain") || !u.test(H + "input")) {
      H('0');
    } else {
      G();
    }
...
let yt = {
  getInfo: async (r, u = {}) => {
    try {
      ...
      let z = [];
      for (let f = 0x0; f < o.streaming_data.formats.length; f++) {
      ...
      let j = {
        ...
        'thumbnail': o.basic_info.thumbnail[0x0].url,
      ...
      console.log("./lib/ROYAL/yt.getInfo()\n", E.message);
      return {
        'status': false
      };
    }
  ...
  "download": async (r, u = {
    ...
    try {
      ...
      console.log("./lib/ROYAL/yt.dowanload()\n", Q.message);
      return false;
    }
  }
};
async function sendAnimeReaction(r, u = "punch", H = '', o = '') {
  try {
    ...
    let f = r.mentionedJid ? r.mentionedJid[0x0] : r.msg.contextInfo.participant || false;
    let E = await GIFBufferToVideoBuffer(j);
    let b = f ? "*@" + r.sender.split('@')[0x0] + " " + H + " @" + f.split('@')[0x0] + '*' : "*@" + r.sender.split('@')[0x0] + " " + o + '*';
    return f ? await r.bot.sendMessage(r.chat, {
      'video': E,
      'gifPlayback': true,
      'mentions': [f, r.sender],
      'caption': b
    ...
    }) : await r.bot.sendMessage(r.chat, {
      'video': E,
      'gifPlayback': true,
      'mentions': [r.sender],
      'caption': b
    ...
    await r.error(K);
    return console.log("./lib/ROYAL.js/sendAnimeReaction()\n", K);
  }
}
async function sendGImages(r, u, H = Config.caption, o = '') {
  try {
    ...
    await r.error(E);
    return console.log("./lib/ROYAL.js/sendGImages()\n", E);
  }
}
async function AudioToBlackVideo(r, u) {
  try {
    ...
    const j = "ffmpeg -f lavfi -i color=c=black:s=1280x720:d=" + z + " -vf \"format=yuv420p\" " + "./temp/blackScreen.mp4";
    await exec(j);
    const f = "ffmpeg -i ./temp/blackScreen.mp4 -i " + r + " -c:v copy -c:a aac -map 0:v:0 -map 1:a:0 " + u;
    await exec(f);
  ...
  try {
    ...
    if (u.toLowerCase() === "install") {
      ...
      for (let K of isUrl(H)) {
        ...
        let v = Q[0x1].split(" ")[0x0] || Math.random().toString(0x24).slice(-0x5);
  ...
  try {
    if (o === 'pp' || o === "gpp") {
      ...
      async function h(j) {
        ...
        const E = f.getWidth();
        const b = f.getHeight();
        const K = f.crop(0x0, 0x0, E, b);
        return {
          'img': await K.scaleToFit(0x144, 0x2d0).getBufferAsync(Jimp.MIME_JPEG),
...
async function forwardMessage(r, u, H = '') {
      ...
      if (o === "imageMessage") {
        ...
        if (o === "audioMessage") {
          ...
          if (o === "documentMessage") {
            ...
            if (o === "conversation" || o === "extendedTextMessage") {
  ...
  if (z) {
    try {
      await ROYAL.bot.relayMessage(r, z, {
        'messageId': u.key.id
      });
    } catch (h) {
      if (H === "ptv" || H === 'save') {
...
async function getRandom(r = ".jpg", u = 0x2710) {
...
async function randomeFunfacts(r) {
  try {
    ...
    await msg.error(h);
    return console.log("./lib/ROYAL.js/randomeFunfacts()\n", h);
  }
}
async function audioEditor(r, u = "bass", H = '') {
  ...
  try {
    ...
    let j = "temp/" + (r.sender.slice(0x6) + u) + ".mp3";
    ...
    await r.error(f + "\n\ncmdName : " + u + "\n");
    return console.log("./lib/ROYAL.js/audioEditor()\n", f);
  }
}
async function send(r, u, H = {
  ...
  try {
    ...
    return console.log("./lib/ROYAL.js/send()\n", f);
  }
}
async function react(r, u, H = '') {
  try {
    ...
    return console.log("./lib/ROYAL.js/react()\n", z);
  }
}
let note = {
...
async function sendWelcome(r, u = '', H = '', o = '', z = "msg", h = true) {
  try {
    ...
    if (u) {
      ...
      if (!z || z === 'msg') {
        ...
        if (/@pp/g.test(u)) {
          ...
          return r.jid && /@gpp/g.test(u) ? await r.send(await r.getpp(r.jid), {
            'caption': j,
            'mentions': o
          }, "image", H) : await r.send(j, {
            'mentions': o
          }, "ROYAL", H);
        }
      } else {
    ...
    return console.log("./lib/ROYAL.js/sendWelcome()\n", E);
  }
}
async function aitts(r, u = '') {
  try {
    ...
    if (!Config.ELEVENLAB_API_KEY || !Config.ELEVENLAB_API_KEY.length > 0x8) {
    ...
    let h = u;
    var H = 0x0 || Math.floor(Math.random() * o.length);
    let j = false;
    if (!isNaN(z) && z > 0x0 && z < 0x27) {
      j = true;
      H = z;
    }
    if (u && u.includes(':')) {
      let b = u.split(':');
      let K = b[b.length - 0x1].trim() || '';
      h = b.slice(0x0, b.length - 0x1).join(':');
      if (K.toLowerCase() === 'richel' || K === '1') {
        H = 0x0;
      } else {
        if (K.toLowerCase() === "clyde" || K === '2') {
          H = 0x1;
        } else {
          if (K.toLowerCase() === 'domi' || K === '3') {
            H = 0x2;
          } else {
            if (K.toLowerCase() === "dave" || K === '4') {
              H = 0x3;
            } else {
              if (K.toLowerCase() === 'fin' || K === '5') {
                H = 0x4;
              } else {
                if (K.toLowerCase() === 'bella' || K === '6') {
                  H = 0x5;
                } else {
                  if (K.toLowerCase() === "antoni" || K === '7') {
                    H = 0x6;
                  } else {
                    if (K.toLowerCase() === "thomas" || K === '8') {
                      H = 0x7;
                    } else {
                      if (K.toLowerCase() === 'charlie' || K === '9') {
                        H = 0x8;
                      } else {
                        if (K.toLowerCase() === "emily" || K === '10') {
                          H = 0x9;
                        } else {
                          if (K.toLowerCase() === "elli" || K === '11') {
                            H = 0xa;
                          } else {
                            if (K.toLowerCase() === "callum" || K === '12') {
                              H = 0xb;
                            } else {
                              if (K.toLowerCase() === "patrick" || K === '13') {
                                H = 0xc;
                              } else {
                                if (K.toLowerCase() === "harry" || K === '14') {
                                  H = 0xd;
                                } else {
                                  if (K.toLowerCase() === "liam" || K === '15') {
                                    H = 0xe;
                                  } else {
                                    if (K.toLowerCase() === "dorothy" || K === '16') {
                                      H = 0xf;
                                    } else {
                                      if (K.toLowerCase() === "josh" || K === '17') {
                                        H = 0x10;
                                      } else {
                                        if (K.toLowerCase() === "arnold" || K === '18') {
                                          H = 0x11;
                                        } else {
                                          if (K.toLowerCase() === "charlotte" || K === '19') {
                                            H = 0x12;
                                          } else {
                                            if (K.toLowerCase() === "matilda" || K === '20') {
                                              H = 0x13;
                                            } else {
                                              if (K.toLowerCase() === 'matthew' || K === '21') {
                                                H = 0x14;
                                              } else {
                                                if (K.toLowerCase() === "james" || K === '22') {
                                                  H = 0x15;
                                                } else {
                                                  if (K.toLowerCase() === "joseph" || K === '23') {
                                                    H = 0x16;
                                                  } else {
                                                    if (K.toLowerCase() === 'jeremy' || K === '24') {
                                                      H = 0x17;
                                                    } else {
                                                      if (K.toLowerCase() === "michael" || K === '25') {
                                                        H = 0x18;
                                                      } else {
                                                        if (K.toLowerCase() === "ethan" || K === '26') {
                                                          H = 0x19;
                                                        } else {
                                                          if (K.toLowerCase() === 'gigi' || K === '27') {
                                                            H = 0x1a;
                                                          } else {
                                                            if (K.toLowerCase() === "freya" || K === '28') {
                                                              H = 0x1b;
                                                            } else {
                                                              if (K.toLowerCase() === 'grace' || K === '29') {
                                                                H = 0x1c;
                                                              } else {
                                                                if (K.toLowerCase() === "daniel" || K === '30') {
                                                                  H = 0x1d;
                                                                } else {
                                                                  if (K.toLowerCase() === 'serena' || K === '31') {
                                                                    H = 0x1e;
                                                                  } else {
                                                                    if (K.toLowerCase() === 'adam' || K === '32') {
                                                                      H = 0x1f;
                                                                    } else {
                                                                      if (K.toLowerCase() === "nicole" || K === '33') {
                                                                        H = 0x20;
                                                                      } else {
                                                                        if (K.toLowerCase() === "jessie" || K === '34') {
                                                                          H = 0x21;
                                                                        } else {
                                                                          if (K.toLowerCase() === "ryan" || K === '35') {
                                                                            H = 0x22;
                                                                          } else {
                                                                            if (K.toLowerCase() === "ROYAL" || K === '36') {
                                                                              H = 0x23;
                                                                            } else {
                                                                              if (K.toLowerCase() === "glinda" || K === '37') {
                                                                                H = 0x24;
                                                                              } else {
                                                                                if (K.toLowerCase() === "giovanni" || K === '38') {
                                                                                  H = 0x25;
...
function G(r) {
  function u(H) {
    ...
    } else if (('' + H / H).length !== 0x1 || H % 0x14 === 0x0) {
      (function () {
        return true;
